...
- [Calculator.gen.php](file://apps/calculator/Application.php)
- [CalculatorLayout_gen.php](file://apps/calculator/main.php)
...
**图表来源**
- [sfc-compiler.php:18-210](file://framework/sfc-compiler.php#L18-L210)
- [aot-validator.php:17-169](file://framework/compiler/aot-validator.php#L17-L169)
- [template-parser.php:60-680](file://framework/compiler/template-parser.php#L60-L680)
...
- [Calculator.gen.php:1-174](file://apps/calculator/Application.php#L1-L174)
- [CalculatorLayout_gen.php:1-296](file://apps/calculator/main.php#L1-L296)
...
- [main.php:26-291](file://apps/calculator/main.php#L26-L291)
...
**图表来源**
- [sfc-compiler.php:184-209](file://framework/sfc-compiler.php#L184-L209)
...
验证规则与实现要点：
- **规则1：文件名点号限制**：禁止在文件名主干部分出现多余点号，避免AOT生成无效C++符号名。
- **规则2：常量数组嵌套限制**：禁止在生成文件中出现const数组包含嵌套结构，避免全局常量未注册导致的问题。
- **规则3：变量属性访问限制**：禁止$obj->$var形式，避免AOT不支持的动态属性访问。
- **规则4：变量方法调用限制**：禁止$obj->$method()形式，避免AOT不支持的动态方法调用。
- **规则5：PHP8函数兼容性**：检测str_contains等函数，给出替代方案提示，避免不同PHP版本差异导致的运行时问题。
- **规则6：顶层声明合法性**：允许顶层const/函数/类声明，但需确保生成文件主体结构符合AOT要求。
- **规则7：变量函数调用限制**：禁止$fn()形式，避免AOT类型推导失败导致的运行时崩溃。
...
**图表来源**
- [template-parser.php:118-199](file://framework/compiler/template-parser.php#L118-L199)
- [template-parser.php:205-279](file://framework/compiler/template-parser.php#L205-L279)
- [template-parser.php:456-541](file://framework/compiler/template-parser.php#L456-L541)
...
- **变量函数调用检测优化**：新加入的变量函数调用检测通过专门的正则表达式优化，排除常见误报模式。
...
- **使用变量方法调用$obj->$method()**
  ...
  - 参考规则：变量方法调用限制。
- **使用变量函数调用$fn()**
  - 现象：报错指出不支持的变量函数调用，AOT类型推导失败。
  - 解决：改用显式的if/else或match分派，或使用callLayoutSegment()等显式函数调用。
  - 参考规则：变量函数调用限制。
...
**新增** 变量函数调用问题的详细说明：
- **问题特征**：当代码中使用 `$fn()` 形式的变量函数调用时，AOT编译器无法正确推断返回值类型，导致运行时崩溃。
- **解决方案**：
  - 使用显式if/else分派：`if ($fn === 'funcA') { funcA(); } elseif ($fn === 'funcB') { funcB(); }`
  - 使用match表达式：`match($fn) { 'funcA' => funcA(), 'funcB' => funcB() };`
  - 使用显式函数调用：`callLayoutSegment($segmentName);`
- **参考实现**：BaseRenderer类中的callLayoutSegment()方法提供了正确的变量函数调用模式。

**章节来源**
- [aot-validator.php:42-106](file://framework/compiler/aot-validator.php#L42-L106)
- [BaseRenderer.php:10-12](file://framework/BaseRenderer.php#L10-L12)
...
**更新** 新增的变量函数调用检测进一步增强了验证器的完整性，解决了AOT编译器在类型推导方面的已知问题，为开发者提供了更全面的AOT兼容性保障。

## 附录

### 验证规则清单
- **规则1：文件名点号限制**：文件名主干最多允许一个点号。
- **规则2：常量数组嵌套限制**：禁止const数组包含嵌套结构。
- **规则3：变量属性访问限制**：禁止$obj->$var。
- **规则4：变量方法调用限制**：禁止$obj->$method()。
- **规则5：PHP8函数兼容性**：检测并提示str_contains等函数的替代方案。
- **规则6：顶层声明合法性**：允许顶层const/函数/类声明，但需保证生成文件结构合规。
- **规则7：变量函数调用限制**：禁止$fn()形式，使用显式函数调用替代。
...
### 测试用例参考
- **变量函数调用测试**：验证器包含针对$fn()形式的测试用例，确保正确识别并拒绝此类调用。
- **变量方法调用测试**：验证器包含针对$obj->$method()形式的测试用例，确保正确识别并拒绝此类调用。
- **变量属性访问测试**：验证器包含针对$obj->$var形式的测试用例，确保正确识别并拒绝此类访问。

**章节来源**
- [sfc-compiler-test.php:357-363](file://tests/sfc-compiler-test.php#L357-L363)
- [sfc-compiler-test.php:350-356](file://tests/sfc-compiler-test.php#L350-L356)
- [sfc-compiler-test.php:365-372](file://tests/sfc-compiler-test.php#L365-L372)